...
You need a working Go installation (1.25.6+) and a clone of the GuLiN repository so the SDK `replace` directive in `go.mod` can resolve locally. The Tsunami build system uses [Task](https://taskfile.dev) for build targets.
...
| `cpuchart` | `task tsunami:demo:cpuchart` | `UseTicker` for periodic polling, recharts integration |
| `tabletest` | `task tsunami:demo:tabletest` | `ui.MakeTableComponent` with sorting and pagination |
| `pomodoro` | `task tsunami:demo:pomodoro` | Timer lifecycle with `UseGoRoutine` |
| `modaltest` | `task tsunami:demo:modaltest` | `UseAlertModal` and `UseConfirmModal` hooks |
